...
if(($handle = fopen('../data/data.csv', 'r')) !== false) {
    while(($data = fgetcsv($handle, 4096, ',')) !== false) {
      $intake = array_search($data[1], $data);
      $date = array_search($data[2], $data);
      $time = array_search($data[3], $data);
      $location = array_search($data[4], $data);
      $classroom = array_search($data[5], $data);
      $module = array_search($data[6], $data);
      $lecterur = array_search($data[7], $data);

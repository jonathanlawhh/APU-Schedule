  ...
  <?php if(isset($searchUser)){ ?>
    ...
    <table class="highlight responsive-table">
      ...
      foreach($returnval AS $array){
        ...
        if($t1 === $result[0] && $t2 === $result[1]){ continue; } else {
          $e0 = trim($result[0]); $e1 = trim($result[1]);
          if($e1==='S6'){ $totalHours += 3; } elseif($e1==='Saturday'){ $totalHours += 9; } else { $totalHours += 2; } //Calculate total hours
          ...
          echo '</tr>'; } $t1 = $result[0]; $t2 = $result[1];
      } //End foreach and update total hours for user
      echo "<script>totalHours($totalHours);</script>"; ?>
      </tbody>
